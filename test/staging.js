...
describe('staging mode', () => {
  ...
  describe('link manipulation', () => {
    ...
    it('handles links generated by to() directives', (done) => {
      nock('http://content')
        .get('/control').reply(200, { sha: null })
        .get('/content/https%3A%2F%2Fgithub.com%2Fbuild-12345%2Fdeconst%2Ffake')
        .reply(200, {
          envelope: { body: '<a href="{{ to(\'https://github.com/build-12345/deconst/fake/other\') }}">x</a>' }
        });

      request(server.create())
        .get('/build-12345/')
        .expect(200)
        .expect(/<a href="\/build-12345\/other\/">x<\/a>/, done);
    });

    it('handles links generated by to() directives to other domains', (done) => {
      nock('http://content')
        .get('/control').reply(200, { sha: null })
        .get('/content/https%3A%2F%2Fgithub.com%2Fbuild-12345%2Fdeconst%2Ffake')
        .reply(200, {
          envelope: { body: '<a href="{{ to(\'https://github.com/build-12345/deconst-dog/fake/other\') }}">x</a>' }
        });
